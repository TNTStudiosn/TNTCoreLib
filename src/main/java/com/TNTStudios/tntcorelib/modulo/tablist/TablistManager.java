...
import net.minecraft.network.packet.s2c.play.PlayerRemoveS2CPacket; // <- Importé el paquete correcto para eliminar jugadores.
import net.minecraft.server.MinecraftServer;
import net.minecraft.server.network.ServerPlayerEntity;

import java.util.Collections; // <- Importé 'Collections' para usar singletonList, más eficiente para un solo elemento.
...
public class TablistManager implements TablistApi {
  ...
  public void hidePlayer(ServerPlayerEntity playerToHide, ServerPlayerEntity observer) {
    ...
    // A partir de la 1.19.4, esto se hace con PlayerRemoveS2CPacket.
    observer.networkHandler.sendPacket(new PlayerRemoveS2CPacket(Collections.singletonList(playerToHide.getUuid())));
  }
  ...
  public void showPlayer(ServerPlayerEntity playerToShow, ServerPlayerEntity observer) {
    ...
    // La acción ADD_PLAYER lo reincorpora y sigue siendo válida.
    ...
    ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
      ...
      for (ServerPlayerEntity onlinePlayer : this.server.getPlayerManager().getPlayerList()) {
        ...
        if (isPlayerHidden(onlinePlayer.getUuid(), newPlayer.getUuid())) {
          newPlayer.networkHandler.sendPacket(new PlayerRemoveS2CPacket(Collections.singletonList(onlinePlayer.getUuid())));
        }
        ...
        if (isPlayerHidden(newPlayer.getUuid(), onlinePlayer.getUuid())) {
          onlinePlayer.networkHandler.sendPacket(new PlayerRemoveS2CPacket(Collections.singletonList(newPlayer.getUuid())));
        }
      }
    });
  }
